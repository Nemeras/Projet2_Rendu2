...
type changing_clauses = (bool*clause) array
...
let init cnf stack current pos solution print =
	
	(* On déduit les conséquences possibles *)
	propa stack current pos solution print ;
...
let solve cnf print =
	...
	let current, pos = cnf_to_vect cnf solution in
	...
	let k = init cnf stack current pos solution print in
	...
	while abs !k <= cnf.v_real && !k <> 0 do
		...
		if abs !k = cnf.v_real then
			...
			if solution.(0) < 0 then
				continue stack current pos solution k back print
			(* Sinon : c'est fini *)
			else
				k := cnf.v_real + 1
		(* Sinon : on continue *)
